...
func mk_parenthesis_unlabeled(labelsP []int) string {
	var d = ""
	var res = &d
	for _, val := range labelsP  {
        var si = strconv.Itoa(val)
        writeLine(mk_rule(mk_stat("Po","op--"+si)),res)
        writeLine(mk_rule("P(X0, cp--"+si+")","Po(X0)"),res)
    }
    return d
}
...
func mk_brackets(labelsB []int) string {
	...
	for _, val := range labelsB  {
        var si = strconv.Itoa(val)
        //if val == exclude_label {
		//	writeLine(mk_rule(mk_stat("S","ob--"+si)),res)
		//	writeLine(mk_rule(mk_stat("S","cb--"+si)),res)
		//}
		//else {
...
func mk_brackets_unlabeled_P(labelsB []int) string {
	var d = ""
	var res = &d
	for _, val := range labelsB  {
        var si = strconv.Itoa(val)
        writeLine(mk_rule(mk_stat("Po","ob--"+si)),res)
        writeLine(mk_rule("P(X0, cb--"+si+")","Po(X0)"),res)
    }
    return d
}
...
func dyck_project_grammar(labelsP []int, labelsB []int) (MCFG, error) {
	var d = ""
	var res = &d
	writeLine(mk_parenthesis_unlabeled(labelsP),res)
	writeLine(mk_brackets_unlabeled_P(labelsB),res)
	writeLine("S(eps).",res)
	writeLine("S(normal).",res)
	writeLine("S(X0 Y0) :- S(X0), S(Y0).",res)
	if len(labelsP) > 0 || len(labelsB) > 0 {
...
func dyck_alpha_grammar_se(labelsP []int, labelsB []int, k int) (MCFG, error) {
	...
	for _, val := range labelsB {
		var si = strconv.Itoa(val)
		writeLine("Sc(cb--"+si+").",res)
		writeLine("So(ob--"+si+").",res)
	}
	if len(labelsP) > 0 {
	...
	for _, c := range []string{"","c"} {
		for _, o := range []string{"", "o"} {
			if len(labelsP) > 0 {
				writeLine("S"+c+o+"(Y0 X0 Y1) :- S"+c+o+"(X0), P(Y0, Y1).",res)
			}
		}
	}
	...
	for _, c1 := range []string{"","c"} {
		for _, o1 := range []string{"", "o"} {
			writeLine("S"+c1+o1+"(X0 Y0) :- S"+c1+o1+"(X0), Se(Y0).",res)
			writeLine("S"+c1+o1+"(X0 Y0) :- Se(X0), S"+c1+o1+"(Y0).",res)
			for _, c2 := range []string{"","c"} {
				for _, o2 := range []string{"", "o"} {
					writeLine("S"+c1+o2+"(X0 Y0) :- S"+c1+o1+"(X0), S"+c2+o2+"(Y0).",res)
				}
			}
		}
	}
...
func dyck_alpha_grammar_k_parity(labelsP []int, labelsB []int, k int) (MCFG, error) {
	...
	for _, val := range labelsB {
		...
		writeLine("S"+p+"(cb--"+si+").",res)
		writeLine("S"+p+"(ob--"+si+").",res)
		emptyNum -= 1<<labelGroup[val]
	}
	if len(labelsP) > 0 {
		writeLine("Se(Y0 X0 Y1) :- Se(X0), P(Y0, Y1).",res)
	...
	for intP := 0;intP < 1<<k ; intP++ {
		p := int_to_par(intP, k)
		if len(labelsP) > 0 {
			writeLine("S"+p+"(Y0 X0 Y1) :- S"+p+"(X0), P(Y0, Y1).",res)
		}
	}
	writeLine("Se(X0 Y0) :- Se(X0), Se(Y0).",res)
	for intP1 := 0;intP1 < 1<<k ; intP1++ {
		p1 := int_to_par(intP1, k)
		writeLine("S"+p1+"(X0 Y0) :- S"+p1+"(X0), Se(Y0).",res)
		writeLine("S"+p1+"(X0 Y0) :- Se(X0), S"+p1+"(Y0).",res)
		for intP2 := 0;intP2 < 1<<k ; intP2++ {
			p2 := int_to_par(intP2, k)
			intP3 := intP1 ^ intP2
			p3 := int_to_par(intP3, k)
			writeLine("S"+p3+"(X0 Y0) :- S"+p1+"(X0), S"+p2+"(Y0).",res)
		}
	}
...
func dyck_alpha_grammar_k_parity_exclude(labelsP []int, labelsB []int, k int, exLabel int) (MCFG, error) {
	...
	writeLine("S"+int_to_par(0, k)+"(cb--"+strconv.Itoa(exLabel)+").",res)
	writeLine("S"+int_to_par(0, k)+"(ob--"+strconv.Itoa(exLabel)+").",res)
...
func dyck_beta_grammar_k_parity(labelsP []int, labelsB []int, k int) (MCFG, error) {
	...
	for _, val := range labelsP {
		...
		emptyNum += 1<<labelGroup[val]
		p := int_to_par(emptyNum, k)
		writeLine("S"+p+"(cp--"+si+").",res)
		writeLine("S"+p+"(op--"+si+").",res)
		emptyNum -= 1<<labelGroup[val]
	}
	if len(labelsB) > 0 {
	...
	for intP := 0;intP < 1<<k ; intP++ {
		p := int_to_par(intP, k)
		if len(labelsB) > 0 {
			writeLine("S"+p+"(Y0 X0 Y1) :- S"+p+"(X0), B(Y0, Y1).",res)
		}
	}
	writeLine("Se(X0 Y0) :- Se(X0), Se(Y0).",res)
	for intP1 := 0;intP1 < 1<<k ; intP1++ {
...
func dyck_beta_grammar_se(labelsP []int, labelsB []int, k int) (MCFG, error) {
	...
	for _, val := range labelsP {
		var si = strconv.Itoa(val)
		writeLine("Sc(cp--"+si+").",res)
		writeLine("So(op--"+si+").",res)
	}
	if len(labelsB) > 0 {
		writeLine("Se(Y0 X0 Y1) :- Se(X0), B(Y0, Y1).",res)
	}
	for _, c := range []string{"","c"} {
		for _, o := range []string{"", "o"} {
			if len(labelsB) > 0 {
				writeLine("S"+c+o+"(Y0 X0 Y1) :- S"+c+o+"(X0), B(Y0, Y1).",res)
			}
		}
	}
	writeLine("Se(X0 Y0) :- Se(X0), Se(Y0).",res)
